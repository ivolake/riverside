...
class BaseNode:
    def __init__(self,
        ...
        self._successful = OrderedDict({}) # хранилище новых пакетов для этого узла (используется только в TCP и только на
                                               # оконечных узлах)
    ...
    def filled_space(self):
        ...
        if len(queue) == 0:
            ...
            queue_sum = sum([p.size for p in queue])
        awaiting = self.awaiting.values()
        if len(awaiting) == 0:
            awaiting_sum = 0
        else:
            awaiting_sum = sum([p.size for p in awaiting])
    ...
    def add_to_awaiting(self, pkt: BasePacket):
        if pkt is not None:
            self._awaiting.update({f'{pkt.sid}.{pkt.pid}.{pkt.id}': pkt})
        else:
            return -1
    ...
    def add_to_successful(self, pkt: BasePacket):
        if pkt is not None:
            self._successful.update({f'{pkt.sid}.{pkt.pid}.{pkt.id}': pkt})
        else:
            return -1
