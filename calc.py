...
def main():
    ...
    while input_type not in ['0', '1']:
        print('Hi. Choose the way to enter graph\n 0. Template file (.txt, .csv., .xls or .xlsx)\n 1. From built-in database\n(Eg: 1): ', end = '')
        input_type = input()
        if input_type not in ['0', '1']:
            print('\nWrong input type.\n')

    graph_type = -1
    while graph_type not in ['0', '1', '2', '3', '4']:
        print('Choose type of reachability\n 0. Standart reachability in digraph\n 1. Vertrex Mixed Reachability with degree of k\n 2. VMRk with weights and masses\n 3. Magnetic Reachability with degree of k\n 4. MnRk with weights and masses\n(Eg: 0): ', end = '')
        graph_type = input()
        if graph_type not in ['0', '1', '2', '3', '4']:
            print('\nWrong graph reachability type.\n')



    if (graph_type == '0') & (input_type == '0'):
        print('Enter name of file, if its on Desktop or in the same directory, as this program (Eg: graph.txt):. \nOtherwise, enter its full path (Eg: C:\\Users\\username\\Desktop\\graph.txt): ', end = '')
        file_name = input()
        digraph = extract(file_name)
        print('\nEnter starting vertex (Eg: 1): ', end = '')
        start = input()
        print('\nChoose goal vertex (Eg: 10): ', end = '')
        goal = input()
        print('\nChoose positions layout\n1. Parabola\n2. Circluar\n3. Random\n(Eg: 1): ', end = '')
        ipos = int(input())
        print('\nName it please: ', end = '')
        dg_name = input()
        ...
        paths = list(dfs_paths(digraph, start, goal))
        drawing_module.draw_graph0(digraph = digraph, digraph_name = dg_name, paths = paths, ipos = ipos)

    elif (graph_type == '1') & (input_type == '0'):
        print('Enter name of file, if its on Desktop or in the same directory, as this program (Eg: graph.txt):. \nOtherwise, enter its full path (Eg: C:\\Users\\username\\Desktop\\graph.txt): ', end = '')
        file_name = input()
        digraph = extract(file_name)
        print('\nChoose incrementation nodes (Eg: 1): ', end = '')
        inc_n = int(input())
        print('\nEnter k (Eg: 2): ', end = '')
        k = int(input())
        print('\nEnter starting vertex (Eg: 1): ', end = '')
        start = input()
        print('\nChoose goal vertex (Eg: 20): ', end = '')
        goal = input()
        print('\nChoose positions layout\n1. Parabola\n2. Circluar\n3. Random\n(Eg: 1): ', end = '')
        ipos = int(input())
        print('\nName it please: ', end = '')
        ...
        (shit1, inc_nodes, shit2) = choose(incrementation_nodes_N = inc_n)
        paths = list(dfs_vmrk_paths(digraph, inc_nodes, k, start, goal))
        drawing_module.draw_graph1(digraph = digraph, digraph_name = dg_name, inc_nodes = inc_nodes, k = k, paths = paths, ipos = ipos)

    elif (graph_type == '2') & (input_type == '0'):
        print('Enter name of file, if its on Desktop or in the same directory, as this program (Eg: graph.txt):. \nOtherwise, enter its full path (Eg: C:\\Users\\username\\Desktop\\graph.txt): ', end = '')
        file_name = input()
        digraph = extract(file_name)
        print('\nChoose incrementation nodes (Eg: 3): ', end = '')
        inc_n = int(input())
        print('\nEnter k (Eg: 2): ', end = '')
        k = int(input())
        print('\nEnter mass (size of package in mbytes) (Eg: 200): ', end = '')
        m = int(input())
        print('\nEnter starting vertex (Eg: 1): ', end = '')
        start = input()
        print('\nChoose goal vertex (Eg: 10): ', end = '')
        ...
        (shit1, inc_nodes, shit2) = choose(incrementation_nodes_N = inc_n)
        path = dfs_vmrk_paths_telecom_nets(digraph, inc_nodes, k, start, goal, m)
        drawing_module.draw_graph2(digraph = digraph, digraph_name = dg_name, inc_nodes = inc_nodes, k = k, path = path[0], time = path[1], mass = m, ipos = ipos)

    elif (graph_type == '3') & (input_type == '0'):
        print('Enter name of file, if its on Desktop or in the same directory, as this program (Eg: graph.txt):. \nOtherwise, enter its full path (Eg: C:\\Users\\username\\Desktop\\graph.txt): ', end = '')
        file_name = input()
        digraph = extract(file_name)
        print('\nChoose incrementation nodes (Eg: 1): ', end = '')
        inc_n = int(input())
        print('\nChoose decrementation nodes (Eg: 1): ', end = '')
        dec_n = int(input())
        print('\nEnter k (Eg: 2): ', end = '')
        k = int(input())
        print('\nEnter starting vertex (Eg: 1): ', end = '')
        ...
        (shit1, inc_nodes, dec_nodes) = choose(incrementation_nodes_N = inc_n, decrementation_nodes_N = dec_n)
        paths = list(dfs_mnrk_paths(digraph, inc_nodes, k, start, goal, dec_nodes))
        drawing_module.draw_graph3(digraph = digraph, digraph_name = dg_name, inc_nodes = inc_nodes, dec_nodes = dec_nodes, k = k, paths = paths, ipos = ipos)

    elif (graph_type == '4') & (input_type == '0'):
        print('Enter name of file, if its on Desktop or in the same directory, as this program (Eg: graph.txt):. \nOtherwise, enter its full path (Eg: C:\\Users\\username\\Desktop\\graph.txt): ', end = '')
        file_name = input()
        digraph = extract(file_name)
        print('\nChoose incrementation nodes (Eg: 1): ', end = '')
        ...
        print('\nEnter k (Eg: 2): ', end = '')
        k = int(input())
        print('\nEnter mass (size of package in mbytes) (Eg: 20): ', end = '')
        m = int(input())
        print('\nEnter checking flag (option)\n 0. Fastest path of current magnetization.\n 1. Fastest path in general.\n(Eg: 0): ', end = '')
        c = int(input())
        print('\nEnter starting vertex (Eg: 1): ', end = '')
        start = input()
        print('\nChoose goal vertex (Eg: 20): ', end = '')
        ...
        (shit1, inc_nodes, dec_nodes) = choose(incrementation_nodes_N = inc_n, decrementation_nodes_N = dec_n)
        path = dfs_mnrk_paths_telecom_nets(digraph, inc_nodes, k, start, goal, m, dec_nodes, check = c)
        drawing_module.draw_graph4(digraph = digraph, digraph_name = dg_name, inc_nodes = inc_nodes, dec_nodes = dec_nodes, k = path[2], path = path[0], time = path[1], mass = m, ipos = ipos)


# ------------------------------------------------------


    elif (graph_type == '0') & (input_type == '1'):
        print('Choose the graph (Eg: 0): ', end = '')
        dg_n = int(input())
        print('\nEnter starting vertex (Eg: 1): ', end = '')
        start = input()
        print('\nChoose goal vertex (Eg: 10): ', end = '')
        ...
        inc_n = 0                                                                     # inc_n = 0 для выгрузки в inc_nodes пустого множества,
        (digraph, inc_nodes) = choose(dg_n,inc_n)                                      # так как в обычном графе запрещенные верины не нужны
        paths = list(dfs_paths(digraph, start, goal))
        drawing_module.draw_graph0(digraph = digraph, digraph_name = dg_name, paths = paths, ipos = ipos)

    elif (graph_type == '1') & (input_type == '1'):
        print('Choose the graph (Eg: 1): ', end = '')
        dg_n = int(input())
        print('\nChoose incrementation nodes (Eg: 1): ', end = '')
        inc_n = int(input())
        print('\nEnter k (Eg: 2): ', end = '')
        ...
        (digraph, inc_nodes) = choose(dg_n,inc_n)
        paths = list(dfs_vmrk_paths(digraph, inc_nodes, k, start, goal))
        drawing_module.draw_graph1(digraph = digraph, digraph_name = dg_name, inc_nodes = inc_nodes, k = k, paths = paths, ipos = ipos)

    elif (graph_type == '2') & (input_type == '1'):
        print('Choose the graph (Eg: 2): ', end = '')
        dg_n = int(input())
        print('\nChoose incrementation nodes (Eg: 3): ', end = '')
        inc_n = int(input())
        print('\nEnter k (Eg: 2): ', end = '')
        ...
        (digraph, inc_nodes, dec_nodes) = choose(digraph_N = dg_n, incrementation_nodes_N = inc_n, decrementation_nodes_N = dec_n)
        paths = list(dfs_mnrk_paths(digraph, inc_nodes, k, start, goal, dec_nodes))
        drawing_module.draw_graph3(digraph = digraph, digraph_name = dg_name, inc_nodes = inc_nodes, dec_nodes = dec_nodes, k = k, paths = paths, ipos = ipos)
